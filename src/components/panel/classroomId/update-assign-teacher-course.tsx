...
const UpdateAssignTeacherCourse = () => {
	...
	const { dataStateAssignTeacherCourse } = useAppSelector((state) => state.classroomId);

	const [updateAssignTeacherCourse, { isLoading }] = useUpdateAssignTeacherAndCourseMutation();

	const form = useForm<z.infer<typeof updateAssignTeacherCourseSchema>>({
		resolver: zodResolver(updateAssignTeacherCourseSchema),
		mode: 'onChange',
		defaultValues: {
			paymentPrice: dataStateAssignTeacherCourse?.paymentPrice?.toString(),
		},
	});

	const onSubmit = async (payload: z.infer<typeof updateAssignTeacherCourseSchema>) => {
		try {
			console.log('updateAssignTeacher -> payload : ', payload);
			console.log('updateAssignTeacher -> id : ', dataStateAssignTeacherCourse.id);

			// return;

			const newPayload = {
				id: dataStateAssignTeacherCourse.classroomCourseId as number,
			...
			dispatch(setModalState({ value: { modalUpdateAssignTeacherCourse: false } }));
			...
			<Form {...form}>
				<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
					<FormItem>
						...
						<Input readOnly value={dataStateAssignTeacherCourse?.courseName} />
					</FormItem>
					<FormItem>
					...
					<FormLib form={form} name="paymentPrice" label="Harga" type="number" />
