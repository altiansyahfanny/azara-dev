...
const TableBrowse = () => {
			...
			if (['meetingDate'].includes(dataIndex)) {
				const value = {
					from: filter?.meetingDate?.startFrom
						? parse(filter?.meetingDate?.startFrom as unknown as string, 'yyyy-LL-dd', new Date())
						: undefined,
					to: filter?.meetingDate?.endTo
						? parse(filter?.meetingDate?.endTo as unknown as string, 'yyyy-LL-dd', new Date())
						: undefined,
				};
					...
					<div className="w-56">
						<InputDateRange
							...
							onSelect={(e) => {
								...
								if (e) {
									if (e.from) {
										startFrom = format(new Date(e.from), 'yyyy-LL-dd');
									}
									if (e.to) {
										endTo = format(new Date(e.to), 'yyyy-LL-dd');
									}
								}
	...
	const columns: TablePropsAntd<Meeting>['columns'] = [
			...
			render: (meeting: Meeting) => {
				return (
					<div className="flex items-center gap-x-2">
						<Table.ButtonAction onClick={() => navigate(`${meeting.id}`)} Icon={Eye} />
						...
						{auth.isAdmin && (
							<Table.ButtonAction
								...
								variant={meeting.isVerified ? 'default' : 'outline'}
							/>
						)}
					</div>
				);
		...
		},
		{
			...
			render: (text: string) => {
				return <p>{format(parse(text, 'HH:mm:ss', new Date()), 'HH:mm')}</p>;
			},
		},
		{
			title: 'Jam Selesai',
	...
	const [deleteMeeting, { isLoading: isLoadingDelete }] = useDeleteMeetingMutation();
	...
	const onSubmittedVerify = async () => {
		...
		try {
			...
			console.log('Verify Meeting -> onFinish -> success : ', result.message);
	...
	const onSubmittedDelete = async () => {
		...
		try {
			console.log('onSubmittedDelete -> idMeeiting : ', idMeetingToDelete);
