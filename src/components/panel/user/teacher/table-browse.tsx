...
import { useGetTeachersQuery, useVerifyTeacherMutation } from '@/api/teacherApi';
...
const TableBrowse: React.FC<TableBrowseProps> = ({ isBrowse = true }) => {
			...
			render: (teacher: Teacher) => {
				return (
					<div className="flex gap-x-2">
						{!isBrowse && <Table.ButtonAction onClick={() => onEnroll(teacher)} Icon={Plus} />}
						{isBrowse && (
							<>
								<Table.ButtonAction
								...
								<Table.ButtonAction
									...
									variant={teacher.isActive ? 'default' : 'outline'}
								/>
							</>
						)}
					</div>
	...
	const onSubmittedVerify = async () => {
		...
		try {
			...
			console.log('Verify Teacher -> onFinish -> success : ', result.message);
