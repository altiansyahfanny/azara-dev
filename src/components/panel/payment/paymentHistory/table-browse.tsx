...
const TableBrowse = () => {
	...
	const [idPaymentHistoryToDelete, setIdPaymentHistoryToDelete] = useState<number | null>();
			...
			if (['paymentDate'].includes(dataIndex)) {
				const value = {
					from: filter?.paymentDate?.startFrom
						? parse(filter?.paymentDate?.startFrom as unknown as string, 'yyyy-LL-dd', new Date())
						: undefined,
					to: filter?.paymentDate?.endTo
						? parse(filter?.paymentDate?.endTo as unknown as string, 'yyyy-LL-dd', new Date())
						: undefined,
				};
	...
	const [deletePaymentHistory, { isLoading: isLoadingDelete }] = useDeletePaymentHistoryMutation();
	...
	const onSubmittedDelete = async () => {
		...
		try {
			console.log('onSubmittedDelete -> idPaymentHistory : ', idPaymentHistoryToDelete);
			...
			console.log('DeletePaymentHistory -> onFinish -> success : ', result.message);
